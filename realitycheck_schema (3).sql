...
CREATE TABLE rewards (
  ...
  -- Direct address for audit/debug — avoids join to contract_versions
  -- Especially useful when tracing a specific reward back to its source
  contract_address      CHAR(42)        NOT NULL DEFAULT '0x0000000000000000000000000000000000000001'
);
...
CREATE INDEX idx_rewards_contract_address    ON rewards (contract_address); -- audit trace per contract
...
-- ============================================================
-- AMENDMENT: MULTI-VERSION CONTRACT SUPPORT
-- Added before Solidity implementation begins.
-- Enables V1 → V2 migration without schema changes.
-- ============================================================

-- Step 1: Track all deployed contract versions
CREATE TABLE contract_versions (
  ...
  notes                 TEXT,                       -- e.g. "Added validator staking"
  PRIMARY KEY (contract_name, version)
);

-- Seed with V1 contract placeholders
-- Update address values after deployment
INSERT INTO contract_versions (contract_name, version, address, notes) VALUES
  ('CoreProtocol',       1, '0x0000000000000000000000000000000000000001', 'V1 launch — Banjara Hills'),
  ('RewardTreasury',     1, '0x0000000000000000000000000000000000000002', 'V1 launch'),
  ('ReputationRegistry', 1, '0x0000000000000000000000000000000000000003', 'V1 launch');

-- Step 2: Add contract_version to chain mirror tables
-- Stores version number (1, 2...) not the full address
-- Join to contract_versions when address is needed

ALTER TABLE posts
  ADD COLUMN contract_version SMALLINT NOT NULL DEFAULT 1
    REFERENCES contract_versions (version)
    -- Note: partial reference — version alone is not unique
    -- Use CHECK constraint instead for simplicity
...
-- Step 3: Update indexer_checkpoints to track per contract address
-- (contract_address column already added in earlier amendment)
-- Seed V1 rows with placeholder addresses
UPDATE indexer_checkpoints
SET contract_address = CASE contract_name
  WHEN 'CoreProtocol'       THEN '0x0000000000000000000000000000000000000001'
  WHEN 'RewardTreasury'     THEN '0x0000000000000000000000000000000000000002'
  WHEN 'ReputationRegistry' THEN '0x0000000000000000000000000000000000000003'
END;

-- Step 4: Index for version-filtered queries
-- Useful when you want "only V2 posts" for analytics
CREATE INDEX idx_posts_contract_version       ON posts        (contract_version);
CREATE INDEX idx_validations_contract_version ON validations  (contract_version);
CREATE INDEX idx_rewards_contract_version     ON rewards      (contract_version);


-- ============================================================
-- HOW TO ADD V2 (reference — not executable now)
-- ============================================================
--
-- When V2 contracts are deployed:
--
-- 1. Insert new rows into contract_versions:
--
--    INSERT INTO contract_versions
--      (contract_name, version, address, notes)
--    VALUES
--      ('CoreProtocol', 2, '0xNEW_ADDRESS', 'V2 — added validator staking'),
--      ('RewardTreasury', 2, '0xNEW_ADDRESS', 'V2 — tiered rewards'),
--      ('ReputationRegistry', 2, '0xNEW_ADDRESS', 'V2 — decay improvements');
--
-- 2. Insert new rows into indexer_checkpoints:
--
--    INSERT INTO indexer_checkpoints
--      (contract_name, contract_address, last_block)
--    VALUES
--      ('CoreProtocol_V2', '0xNEW_ADDRESS', <deploy_block>),
--      ('RewardTreasury_V2', '0xNEW_ADDRESS', <deploy_block>),
--      ('ReputationRegistry_V2', '0xNEW_ADDRESS', <deploy_block>);
--
-- 3. Mark V1 contracts as deprecated:
--
--    UPDATE contract_versions
--    SET is_active = FALSE, deprecated_at = NOW()
--    WHERE version = 1;
--
-- 4. Update zone config activeContractVersion = 2
--    (via admin panel → indexer /config endpoint)
--    Frontend automatically posts to V2 on next load.
--
-- 5. Indexer starts new listeners for V2 addresses.
--    V1 listeners continue reading (read-only, no new events).
--
-- No schema changes. No frontend changes. No data migration.
-- V1 rewards still claimable from V1 RewardTreasury forever.
--
-- ============================================================
...
-- ============================================================
-- SCHEMA STATUS: FULLY LOCKED ✓
-- Version: 1.0 — Ready for Solidity implementation
--
-- Build order:
--   Step 1: RealityCheckCoreV1     (post creation + validation)
--   Step 2: Finalization logic      (in CoreV1)
--   Step 3: RewardTreasuryV1        (accounting + payouts)
--   Step 4: Claim logic             (in RewardV1)
--   Step 5: ReputationRegistryV1    (scoring, lightweight)
--
-- Invariant confirmed:
--   No off-chain component can force money movement
